module mod_montecarlo_path_sampler
    ...
    implicit none
    integer :: n_resolution, r_operative_factor, user_seed, n_paths
    integer(kind=4) :: n_operative, i
    logical :: debug
    real(kind=8), allocatable :: brownian_path(:), sample_path(:)
    real(kind=8), allocatable :: montecarlo_sample(:, :)
    ...
    m = 1.5
    sigma = 0.1
    allocate(montecarlo_sample(n_paths, n_operative))
    montecarlo_sample = 0.0
    print*, "First array cache", montecarlo_sample(1:10, 1)    
